"""
Enrique Martín <emartinm@ucm.es> 2024
...
ASIGS = [
    {"nombre": "GIW", "numero_alumnos": 10,
     "horario": [{"dia": "lunes", "hora_inicio": 9, "hora_final": 10}]},
    {"nombre": "KLO", "numero_alumnos": 30,
     "horario": [{"dia": "martes", "hora_inicio": 11, "hora_final": 12}]},
    {"nombre": "POL", "numero_alumnos": 60,
     "horario": [{"dia": "miercoles", "hora_inicio": 12, "hora_final": 14}]},
    {"nombre": "SGD", "numero_alumnos": 75,
     "horario": [{"dia": "jueves", "hora_inicio": 16, "hora_final": 17}]},
    {"nombre": "OMG", "numero_alumnos": 90,
     "horario": [{"dia": "viernes", "hora_inicio": 18, "hora_final": 20}]},
]
...
BAD_ASIGS = [
    ...
    {"nombre": "JUL", "numero_alumnos": 90,
     "horario": [{"dia": "miercoles", "hora_inicio": 16, "hora_final": 17}],
     "campo_adicional": 28},
    {"nombre": 3, "numero_alumnos": 20,
     "horario": [{"dia": "miercoles", "hora_inicio": 16, "hora_final": 17}]},
    {"nombre": "FAL", "numero_alumnos": "hola",
     "horario": [{"dia": "miercoles", "hora_inicio": 16, "hora_final": 17}]},
    # Horario mal formado
    {"nombre": "POL", "numero_alumnos": 12, "horario": 58},
    {"nombre": "OMG", "numero_alumnos": 90,
     "horario": [{"dia": 5, "hora_inicio": 18, "hora_final": 20}]},
    {"nombre": "OMG", "numero_alumnos": 90,
     "horario": [{"dia": "viernes", "hora_inicio": "seis", "hora_final": 20}]},
    {"nombre": "OMG", "numero_alumnos": 90,
     "horario": [{"dia": "viernes", "hora_inicio": 18, "hora_final": "ocho"}]},
    {"nombre": "OMG", "numero_alumnos": 90,
     "horario": [{"dia": "viernes", "hora_inicio": "", "hora_final": 20}]},
    {"nombre": "OMG", "numero_alumnos": 90,
     "horario": [{"dia": "viernes", "hora_inicio": 18, "hora_final": True}]},
    {"nombre": "OMG", "numero_alumnos": 90,
     "horario": [{"dia": "viernes", "hora_inicio": 18, "hora_final": False}]},
    {"nombre": "OMG", "numero_alumnos": 90,
     "horario": [{"day": "viernes", "hora_inicio": 18, "hora_final": 8}]},
    {"nombre": "OMG", "numero_alumnos": 90,
     "horario": [{"hora_inicio": 18, "hora_final": 8}]},
    {"nombre": "OMG", "numero_alumnos": 90,
     "horario": [{"dia": "viernes", "hora_inicial": 18, "hora_final": 20}]},
    {"nombre": "OMG", "numero_alumnos": 90,
     "horario": [{"dia": "viernes", "hora_final": 20}]},
    {"nombre": "OMG", "numero_alumnos": 90,
     "horario": [{"dia": "viernes", "hora_inicio": 18, "hora_finalizacion": 20}]},
    {"nombre": "OMG", "numero_alumnos": 90,
     "horario": [{"dia": "viernes", "hora_inicio": 18}]},
...
class TestREST(unittest.TestCase):
    ...
    def borra_e_inserta(self, list_asignaturas):
        ...
        requests.delete(f'{HOST}/asignaturas', timeout=TIMEOUT)
        ids = []
    ...
    def test_delete_asignaturas(self):
        ...
        r = requests.delete(f'{HOST}/asignaturas', timeout=TIMEOUT)
        self.assertEqual(r.status_code, 204)

    def test_delete_get_asignaturas(self):
        """Después de borrar /asignaturas, el siguiente GET no devuelve ninguna"""
        requests.delete(f'{HOST}/asignaturas', timeout=TIMEOUT)
        r = requests.get(f'{HOST}/asignaturas', timeout=TIMEOUT)
        self.assertEqual(r.json()['asignaturas'], [])

    def test_post_get_asignaturas(self):
        """Añadir varias asignaturas después de borrarlas todas: el código de
        retorno es correcto, el JSON devuelto tiene únicamente la clave 'id'
        y el siguiente GET devuelve URL a todas las asignaturas"""
        added = self.borra_e_inserta(ASIGS)

        r = requests.get(f'{HOST}/asignaturas', timeout=TIMEOUT)
        self.assertEqual(r.status_code, 200)
        self.assertEqual(len(r.json()['asignaturas']), len(ASIGS))
        self.assertEqual(set(r.json()['asignaturas']),
                         {f"/asignaturas/{asig['id']}" for asig in added})

    def test_post_get_filtro(self):
        """Añadir varias asignaturas después de borrarlas todas: el siguiente
        GET devuelve correctamente las asignaturas teniendo en cuenta el filtro"""
        added = self.borra_e_inserta(ASIGS)

        r = requests.get(f'{HOST}/asignaturas?alumnos_gte=5', timeout=TIMEOUT)
        self.assertEqual(r.status_code, 200)
        self.assertEqual(len(r.json()['asignaturas']), len(ASIGS))

        r = requests.get(f'{HOST}/asignaturas?alumnos_gte=1000', timeout=TIMEOUT)
        ...
        for umbral in range(10, 100, 10):
            ...
            r = requests.get(f'{HOST}/asignaturas?alumnos_gte={umbral}', timeout=TIMEOUT)
        ...
        for page in range(1, 10):
            for per_page in range(1, 10):
                for umbral in range(10, 150, 10):
                    r = requests.get(
                        f'{HOST}/asignaturas?per_page={per_page}'
                        f'&page={page}&alumnos_gte={umbral}',
                        timeout=TIMEOUT)
                    expected = set(rutas_paginado(
                        list(filter(lambda x: x['numero_alumnos'] >= umbral, added)),
                        per_page, page))
                    expected_code = 200 if len(expected) == len(added) else 206
                    self.assertEqual(r.status_code, expected_code)
                    self.assertEqual(set(r.json()['asignaturas']), expected)
    ...
    def test_post_get_asignatura(self):
        ...
        for ident in range(10):
            r = requests.get(f'{HOST}/asignaturas/{ident}', timeout=TIMEOUT)
            self.assertEqual(r.status_code, 404)

        added = self.borra_e_inserta(ASIGS)
        for asig in added:
            r = requests.get(f'{HOST}/asignaturas/{asig["id"]}', timeout=TIMEOUT)
            self.assertEqual(r.status_code, 200)
    ...
    def test_delete_asignatura(self):
        ...
        self.borra_e_inserta([])
        for ident in range(10):
            r = requests.delete(f'{HOST}/asignaturas/{ident}', timeout=TIMEOUT)
            self.assertEqual(r.status_code, 404)
    ...
    def test_put_asignatura_mal_formada(self):
        """Reemplazar una asignatura usando datos mal formados debe
        obtener 404 si no existe y 400 si existe"""
        self.borra_e_inserta([])
        for ident in range(10):
    ...
    def test_patch_asignatura(self):
        """Actualizar un campo de una asignatura que y luego obtenerla
        debe devolver los datos actualizados"""
        added = self.borra_e_inserta(ASIGS)
        for asig in added:
            for patch in PATCH:
                r = requests.get(f'{HOST}/asignaturas/{asig["id"]}', timeout=TIMEOUT)
    ...
    def test_patch_asignatura_mal_formada(self):
        """Actualizar un campo de una asignatura usando datos mal formados
        obtiene 404 si la asignatura no existe, y 400 si existe"""
        self.borra_e_inserta([])
        for ident in range(10):
